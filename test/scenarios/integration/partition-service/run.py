...
TESTS = {"TestPartitionNativeQueueReplay", "TestPartitionNativeCatalogReplay", "TestPartitionNativeAcknowledgedMovement", "TestPartitionNativeObsoleteOpenCompletion", "TestPartitionNativeShardReplay", "TestPartitionNativeLateOpenFencesCurrent"}
...
def main():
    ...
    report = {"schema": 1, "backend": cfg["backend"], "project": project, "result": "failed",
              ...
              "limitations": ["MinIO is not real AWS qualification", "Two partition services in one test process, not cmd/xenon or embedded Temporal acceptance", "Coordinator replacement is an explicit production control CAS; no election failure detector or process crash is simulated", "Real Rust/OS/S3 timing, not deterministic simulation", "Native cases cover shard, cluster, Nexus, namespace, queue and QueueV2 persistence families; not all persistence families or SDK/runtime acceptance"]}
    ...
    try:
        ...
        for folder in (HERE, ROOT / "internal/partitions", ROOT / "internal/cluster", ROOT / "internal/registry", ROOT / "internal/identity", ROOT / "internal/persistence", ROOT / "api/xenon/v1"):
