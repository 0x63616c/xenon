...
This component has **not been run merely by adding the mode**. A future exact clean runtime receipt is required. Its result remains `full_acceptance=false`: there are no injected faults, independent attempted-start census, complete reconstruction of arbitrary result contents, or complete frozen acceptance measurements in this mode. The controller now also runs the source-derived mixed history oracle after workload completion, checking 240 histories and parent/child/CAN links; this is still pending live execution. Its existing 15-minute audit limit is separate from the unchanged 900-second workload limit.
