...
## Accepted ingress and internal forwarding

Calum selected a single service endpoint for the Temporal persistence adapter. In Kubernetes this may be a normal Service over ready Xenon pods; other deployments may use an equivalent load balancer. This does not change the selected Compose proof harness or require Kubernetes for the initial proof. All Xenon nodes run the same Go binary and accept requests for every logical partition. They execute locally only under valid ownership, otherwise forwarding the complete operation to its owner. The application SDK continues to connect to Temporal's frontend.

The internal directory remains authoritative in S3, with disposable routing caches. Preserve the same operation ID, canonical-input digest and end-to-end deadline through forwarding and retry. Bound forwarding hops and retry attempts, refresh stale routes, and return typed unavailability if safe routing cannot be established. Do not allow proxy loops, duplicate mutations, or non-owner execution. The owner applies its normal admission, fencing and durable acknowledgement rules.

Keep node addresses out of durable data and pagination tokens. Direct adapter-to-owner routing can later remove an extra hop without changing storage formats. Long-lived gRPC connections may distribute ingress unevenly; measure before introducing client-side balancing or an additional proxy tier. There is no separate Xenon frontend fleet or new transaction coordinator in the accepted initial design.

Validation remains required: send a request through a non-owner, verify exactly one owner execution and preserved replay after response loss, then move ownership under load and verify stale-route refresh, bounded failure and eventual progress. This section records the accepted design, not completed routing implementation.
