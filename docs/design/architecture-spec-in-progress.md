...
## Delegated placement and layout decision (2026-09-06)

The [measured implementation decision](placement-layout-decision.md) updates the earlier candidate ledger: pinned bounded consistent placement, existing indivisible database domains, one persisted active move, and one bounded control record. Database count, runtime cadence and approximately 100-server capacity remain qualification work. The [composed research receipt](../research/feasibility-batch-20260906.json) records corrected native and receipt-aware contention runs on the reviewed source; it does not prove production service integration.
