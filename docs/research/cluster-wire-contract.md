...
Run `go test -race ./internal/adapter -run '^TestCluster' -count=1`. The two named tests cover every method, deterministic protobuf roundtrip/digest, optional empty filters, max-int64 version/expiry, unknown encoding values, binary blobs/tokens, IPv4/IPv6 and nanosecond timestamps, false result/CAS typed error, transport error conversion, identity-preserving retry and bounded deadline. These are deterministic contract tests with a mocked RPC client, not a native-library experiment. `scripts/generate-proto.sh` includes the new contract for CI generation-drift checking.
