...
PLACEMENT_RESULTS="$evidence/placement.jsonl" go -C "$root/benchmarks/placement" test -race -v -count=1 -timeout=90s ./... | tee "$evidence/placement-tests.txt"
PLACEMENT_RESULTS="$evidence/placement-repeat.jsonl" go -C "$root/benchmarks/placement" test -count=1 -timeout=90s ./... >"$evidence/placement-repeat.txt"
cmp "$evidence/placement.jsonl" "$evidence/placement-repeat.jsonl"
CONTROL_RECORD_RESULTS="$evidence/control-sizes.json" go -C "$root" test -race -v -count=1 -timeout=90s ./benchmarks/storage | tee "$evidence/storage-tests.txt"
go -C "$root/benchmarks/placement" test -run='^$' -bench=BenchmarkPlan -benchmem -benchtime=100x -count=3 -timeout=90s ./... | tee "$evidence/placement-bench.txt"
go -C "$root" test -run='^$' -bench=BenchmarkControlRecord -benchmem -benchtime=100x -count=3 -timeout=90s ./benchmarks/storage | tee "$evidence/storage-bench.txt"
printf 'passed\n' >"$evidence/status"
