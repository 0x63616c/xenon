...
INPUTS = ["tools/ministack.json", "proof/ministack/case.json", "proof/ministack/README.md", "deploy/ministack/compose.json", "deploy/ministack/haproxy.cfg", "deploy/ministack/temporal-a.json", "deploy/ministack/temporal-b.json", "cmd/xenon-config-check/main.go", "scripts/check-ministack.py", "go.mod", "go.sum"]
def digest(path):
...
def main():
    ...
    try:
        ...
        commands=[
            ["go","run","./cmd/xenon-config-check","deploy/ministack/temporal-a.json","deploy/ministack/temporal-b.json"],
            ["docker","compose","-f","deploy/ministack/compose.json","config","--quiet"],
            ["docker","run","--rm","--add-host","host.docker.internal:host-gateway","-v",str(ROOT/"deploy/ministack/haproxy.cfg")+":/usr/local/etc/haproxy/haproxy.cfg:ro",pins["proxy"]["image"],"haproxy","-c","-f","/usr/local/etc/haproxy/haproxy.cfg"],
        ]
        for index,argv in enumerate(commands):
            output=execute(argv)
            if index==0 and output.splitlines()!=["CONFIG_VALID deploy/ministack/temporal-a.json","CONFIG_VALID deploy/ministack/temporal-b.json"]:raise ValueError("missing Temporal config assertions")
