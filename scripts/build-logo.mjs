...
const contents=s=>s.replace(/^[\s\S]*?<svg\b[^>]*>/,'').replace(/<\/svg>\s*$/,'');
const wordmark=readFileSync(path.join(out,'wordmark-white.svg'),'utf8');
const wordmarkViewBox=wordmark.match(/viewBox="([^"]+)"/)[1];
writeFileSync(path.join(out,'banner-counter.svg'),`<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="300" viewBox="0 0 1200 300" role="img" aria-label="Xenon"><title>Xenon</title><rect width="1200" height="300" fill="#000000"/><svg x="52" y="42" width="216" height="216" viewBox="0 0 512 512">${contents(counter)}</svg><svg x="316" y="90" width="730" height="120" viewBox="${wordmarkViewBox}">${contents(wordmark)}</svg></svg>\n`);
