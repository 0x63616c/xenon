#!/usr/bin/env python3
"""Run actual process-cut durability against a scoped S3 emulator."""
import json
import os
...
def main():
    ...
    project = os.environ["XENON_PROCESS_CUT_PROJECT"]
    if not re.fullmatch(r"xenon-process-cut-[a-f0-9]{12}", project):
        raise RuntimeError("invalid scoped project")
    env = os.environ.copy()
    env.update({"AWS_ACCESS_KEY_ID":"xenon-local", "AWS_SECRET_ACCESS_KEY":"xenon-local-test-only", "AWS_REGION":"us-east-1", "AWS_ENDPOINT":cfg["endpoint"], "AWS_ALLOW_HTTP":"true", "AWS_EC2_METADATA_DISABLED":"true", "AWS_CONFIG_FILE":os.devnull, "AWS_SHARED_CREDENTIALS_FILE":os.devnull, "XENON_BUCKET":cfg["bucket"], "XENON_NODE_BINARY":str(ROOT / ".local/bin/xenon-go-node")})
    for name in ("AWS_PROFILE", "AWS_SESSION_TOKEN"):
        env.pop(name, None)
    compose = ["docker", "compose", "--project-name", project, "-f", "deploy/process-cut.compose.yaml"]
    ...
    try:
        ...
        for argv in (["docker","ps","-aq","--filter","label=com.docker.compose.project="+project],["docker","volume","ls","-q","--filter","label=com.docker.compose.project="+project]):
            if execute(argv,env): raise RuntimeError("scoped process-cut resources survived cleanup")

if __name__=="__main__":
    def interrupted(signum, frame): raise RuntimeError(f"process-cut controller interrupted: {signum}")
