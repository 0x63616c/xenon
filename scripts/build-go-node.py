...
report={'source_commit':pins['source_commit'],'source_clean':True,'source_cargo_lock_sha256':prove.digest(source/'Cargo.lock'),'binding_module':pins['go_module'],'binding_version':module['Version'],'binding_sum':module['Sum'],'shared_library':str(library.relative_to(ROOT)),'shared_library_sha256':prove.digest(library),'node_binary':str(output.relative_to(ROOT)),'node_binary_sha256':prove.digest(output),'rustc':run(['rustc','+'+pins['rust_toolchain'],'-vV'],capture=True),'go':run(['go','version'],capture=True),'cc':run(['cc','--version'],capture=True)}
