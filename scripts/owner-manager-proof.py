#!/usr/bin/env python3
"""Run actual conditional owner-manager storage against a scoped S3 emulator."""
import json
import os
...
def main():
    cfg = json.loads((ROOT / "proof/owner-manager/case.json").read_text())
    if cfg["schema"] != 1 or cfg["backend"] != "s3-emulator" or cfg["endpoint"] != "http://127.0.0.1:19004":
        raise RuntimeError("unregistered owner-manager fixture")
    project = os.environ["XENON_OWNER_MANAGER_PROJECT"]
    if not re.fullmatch(r"xenon-owner-manager-[a-f0-9]{12}", project):
    ...
    compose = ["docker", "compose", "--project-name", project, "-f", "deploy/owner-manager.compose.yaml"]
    ...
    try:
        ...
        while True:
            try:
                ...
                time.sleep(0.1)
        completed = subprocess.run(["go","test","-race","-json","-tags","integration_s3","-count=1","-timeout","180s","./internal/ownership","-run","^(TestS3OwnerManager|TestManagedServiceCoverage)$"],cwd=ROOT,env=env,timeout=200)
        if completed.returncode: raise RuntimeError("owner manager assertion failed")
    finally:
        ...
        for argv in (["docker","ps","-aq","--filter","label=com.docker.compose.project="+project],["docker","volume","ls","-q","--filter","label=com.docker.compose.project="+project]):
            if execute(argv,env): raise RuntimeError("scoped owner-manager resources survived cleanup")

if __name__=="__main__":
    def interrupted(signum, frame): raise RuntimeError(f"owner-manager controller interrupted: {signum}")
