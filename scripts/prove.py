...
def command(spec):
    ...
    runner = spec["runner"]
    if runner not in ("cargo-test", "cargo-test-node", "go-test-shard", "go-test-node") or not isinstance(spec["exact"], bool):
...
def main():
    ...
    parser.add_argument("name", choices=["primitive", "ownership", "shard", "go-bindings", "go-shard", "go-namespace", "go-queue"])
    ...
    if args.name in ("go-shard", "go-namespace", "go-queue") and (not commands or commands[0][2] != "go-node-build"):
    ...
    if args.name in ("go-shard", "go-namespace", "go-queue"):
    ...
    try:
        ...
        for index, (argv, expected, runner) in enumerate(commands, 1):
            ...
            if runner == "go-node-build":
                ...
                binary = ROOT / (".local/bin/xenon-go-node" if args.name in ("go-shard", "go-namespace", "go-queue") else "target/debug/xenon-node")
