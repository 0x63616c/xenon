...
def command(spec):
    ...
    runner = spec["runner"]
    if runner not in ("cargo-test", "cargo-test-node", "go-test-shard", "go-test-node", "s3-crash", "s3-crash-cleanup", "go-test-routing", "go-shard-compat", "s3-directory") or not isinstance(spec["exact"], bool):
...
def main():
    ...
    parser.add_argument("name", choices=["primitive", "ownership", "shard", "crash", "go-bindings", "go-shard", "go-namespace", "go-cluster", "go-queue", "go-history", "go-nexus", "go-matching", "go-matching-userdata", "go-fair", "go-queuev2", "go-persistence", "go-shard-compat", "forwarding", "directory"])
    ...
    if manifest["schema"] != 1 or manifest["name"] != args.name or manifest["backend"] != ("s3-emulator" if args.name in ("crash", "go-shard-compat", "directory") else "memory"):
    ...
    if args.name in ("go-shard", "go-namespace", "go-cluster", "go-queue", "go-history", "go-nexus", "go-matching", "go-matching-userdata", "go-fair", "go-queuev2", "go-persistence", "go-shard-compat") and (not commands or commands[0][2] != "go-node-build"):
    ...
    if args.name in ("go-shard", "go-namespace", "go-cluster", "go-queue", "go-history", "go-nexus", "go-matching", "go-matching-userdata", "go-fair", "go-queuev2", "go-persistence", "go-shard-compat"):
    ...
    try:
        ...
        for tool in ("git", "rustc", "cargo", "python", *(["go", "protoc"] if args.name in ("shard", "go-shard-compat") else (["go"] if args.name in ("go-shard", "go-namespace", "go-cluster", "go-queue", "go-history", "go-nexus", "go-matching", "go-matching-userdata", "go-fair", "go-queuev2", "go-persistence", "go-shard-compat", "forwarding", "directory") else []))):
        ...
        for index, (argv, expected, runner) in enumerate(commands, 1):
            ...
            if runner == "go-node-build":
                ...
                binary = ROOT / (".local/bin/xenon-go-node" if args.name in ("go-shard", "go-namespace", "go-cluster", "go-queue", "go-history", "go-nexus", "go-matching", "go-matching-userdata", "go-fair", "go-queuev2", "go-persistence", "go-shard-compat") else "target/debug/xenon-node")
