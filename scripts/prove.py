...
def command(spec):
    ...
    runner = spec["runner"]
    if runner not in ("cargo-test", "cargo-test-node", "go-test-shard", "go-test-node", "s3-crash", "s3-crash-cleanup", "go-test-routing", "go-shard-compat", "s3-directory", "s3-owner-manager") or not isinstance(spec["exact"], bool):
    ...
    if runner == "go-test-shard":
        if not spec["exact"] or spec["filter"] not in ("TestShardRPC", "TestShardTransportBoundsAndTypes", "TestNamespaceRPC", "TestNamespaceByteBoundedPagination", "TestQueueRPC", "TestQueueByteBoundedPagination", "TestQueueRemoteCancellation", "TestHistoryRPC", "TestHistoryTimeoutTypes", "TestHistoryByteBoundedPagination", "TestNexusTransport"):
...
def main():
    ...
    parser.add_argument("name", choices=["primitive", "ownership", "shard", "crash", "go-bindings", "go-shard", "go-namespace", "go-cluster", "go-queue", "go-history", "go-nexus", "go-matching", "go-matching-userdata", "go-queuev2", "go-persistence", "go-shard-compat", "forwarding", "directory", "owner-manager"])
    ...
    if manifest["schema"] != 1 or manifest["name"] != args.name or manifest["backend"] != ("s3-emulator" if args.name in ("crash", "go-shard-compat", "directory", "owner-manager") else "memory"):
    ...
    if args.name in ("go-shard", "go-namespace", "go-cluster", "go-queue", "go-history", "go-nexus", "go-matching", "go-matching-userdata", "go-queuev2", "go-persistence", "go-shard-compat", "owner-manager") and (not commands or commands[0][2] != "go-node-build"):
    ...
    if args.name in ("go-shard", "go-namespace", "go-cluster", "go-queue", "go-history", "go-nexus", "go-matching", "go-matching-userdata", "go-queuev2", "go-persistence", "go-shard-compat", "owner-manager"):
    ...
    try:
        ...
        for tool in ("git", "rustc", "cargo", "python", *(["go", "protoc"] if args.name in ("shard", "go-shard-compat") else (["go"] if args.name in ("go-shard", "go-namespace", "go-cluster", "go-queue", "go-history", "go-nexus", "go-matching", "go-matching-userdata", "go-queuev2", "go-persistence", "go-shard-compat", "forwarding", "directory", "owner-manager") else []))):
        ...
        for index, (argv, expected, runner) in enumerate(commands, 1):
            ...
            if runner == "go-node-build":
                ...
                binary = ROOT / (".local/bin/xenon-go-node" if args.name in ("go-shard", "go-namespace", "go-cluster", "go-queue", "go-history", "go-nexus", "go-matching", "go-matching-userdata", "go-queuev2", "go-persistence", "go-shard-compat", "owner-manager") else "target/debug/xenon-node")
        ...
        if args.name in ("directory", "owner-manager"):
            env_key = "XENON_DIRECTORY_PROJECT" if args.name == "directory" else "XENON_OWNER_MANAGER_PROJECT"
