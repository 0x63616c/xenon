...
def main():
    ...
    parser.add_argument("name", choices=["primitive", "ownership", "shard", "crash", "go-bindings", "go-shard", "go-namespace", "go-cluster", "go-queuev2"])
    ...
    if args.name in ("go-shard", "go-namespace", "go-cluster", "go-queuev2") and (not commands or commands[0][2] != "go-node-build"):
    ...
    if args.name in ("go-shard", "go-namespace", "go-cluster", "go-queuev2"):
    ...
    try:
        ...
        for tool in ("git", "rustc", "cargo", "python", *(["go", "protoc"] if args.name == "shard" else (["go"] if args.name in ("go-shard", "go-namespace", "go-cluster", "go-queuev2") else []))):
        ...
        for index, (argv, expected, runner) in enumerate(commands, 1):
            ...
            if runner == "go-node-build":
                ...
                binary = ROOT / (".local/bin/xenon-go-node" if args.name in ("go-shard", "go-namespace", "go-cluster", "go-queuev2") else "target/debug/xenon-node")
