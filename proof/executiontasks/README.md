...
Six commands cover the actual DLQ RPC, unchanged pinned Temporal task suite (15 tests), native batch rollback/reopen/fence and 3 MiB pages, plus the shared workflow missing-shard regression. The fixture declares the response-loss fault, IDs, limits and upstream random seed. Source scope and the delegated MaxInt64 emptiness correction are in `docs/research/executiontasks-contract.md`.
